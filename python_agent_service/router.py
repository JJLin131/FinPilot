...
from python_agent_service.intents import INTENT_KEYWORDS, INTENT_ORDER
from python_agent_service.llm import IntentClassificationService
from python_agent_service.models import RouteDecision
...
class IntentRouter:
    ...
    def _heuristic_classify(self, user_message: str) -> tuple[str, str]:
        ...
        for intent in INTENT_ORDER:
            if any(keyword in lowered for keyword in [entry.lower() for entry in INTENT_KEYWORDS.get(intent, [])]):
    ...
    def route(self, user_message: str) -> RouteDecision:
        ...
        target_agent = "TransferAgent" if classifier_intent.startswith("TRANSFER") or classifier_intent == "CANCEL_TRANSFER" else "QueryAgent"
        return RouteDecision(
            raw_intent_json=raw_intent_json,
            normalized_intent=classifier_intent,
            reason=reason,
            confidence=round(max(confidence, ROUTE_THRESHOLD), 4),
            valid=True,
