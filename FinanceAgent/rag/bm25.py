...
from FinanceAgent.rag.models import KnowledgeDocumentRequest
...
class Bm25ChunkIndex:
    ...
    def search(self, domain: str, tenant_id: str, query: str, limit: int) -> list[RagMatch]:
        docs = [
            doc
            for doc in self._load()
            if doc.domain == domain and doc.tenant_id in {"__GLOBAL__", tenant_id}
        ]
        if not docs:
            return []
    ...
    def _load(self) -> list[Bm25Document]:
        ...
        return [Bm25Document(**item) for item in payload]
