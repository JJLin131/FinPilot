...
class VectorCandidateRetriever:
    ...
    def retrieve(self, domain: str, tenant_id: str, query: str, limit: int) -> list[RagMatch]:
        if domain != "FINANCE":
            return []
        query_embedding = self.embedding_client.embed(query)
        return self.vector_store.query(query_embedding, limit, tenant_id)
...
class Bm25CandidateRetriever:
    ...
    def retrieve(self, domain: str, tenant_id: str, query: str, limit: int) -> list[RagMatch]:
        return self.index.search(domain, tenant_id, query, limit)
...
class HybridCandidateRetriever:
    ...
    def retrieve(self, domain: str, tenant_id: str, query: str, limit: int) -> list[RagMatch]:
        ...
        self._add_safely(fusion, lambda: self.vector_retriever.retrieve(domain, tenant_id, query, limit), "vector")
        self._add_safely(fusion, lambda: self.bm25_retriever.retrieve(domain, tenant_id, query, limit), "bm25")
        return [
            RagMatch(
